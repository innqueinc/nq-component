    ...
    if (events.length === 1) {
      ...
      switch (event.type) {
        ...
        case 'pointermove':
          ...
          this.lastX = event.x;
          this.lastY = event.y; // get how many percent change from the size of canvas

          const normalX = deltaX / this.cropper.width;
          const normalY = deltaY / this.cropper.height; // convert the percent to the actual width of photo

          const addedX = this.cropper.photo.getWidth() * normalX;
          const addedY = this.cropper.photo.getHeight() * normalY; // add the changes to the current position of photo

...
class Cropper {
  ...
  setPhoto(width, height, scale) {
    ...
    let newWidth = this.clip.width; // calculate ratio

    const clipRatio = this.clip.width / this.clip.height;
    const photoRatio = width / height; // adjust size base on ratio
  ...
  scalePhoto(scale) {
    ...
    const currentHeight = this.photo.height * scale; // minimum size
  ...
  positionPhoto() {
    if (this.photo.x >= this.clip.x) {
      // left
      this.photo.x = this.clip.x;
    }

    if (this.photo.x + this.photo.getWidth() <= this.clip.x + this.clip.width) {
      // right
      this.photo.x = this.clip.x + this.clip.width - this.photo.getWidth();
    }

    if (this.photo.y >= this.clip.y) {
      // top
      this.photo.y = this.clip.y;
    }

    if (this.photo.y + this.photo.getHeight() <= this.clip.y + this.clip.height) {
      // down

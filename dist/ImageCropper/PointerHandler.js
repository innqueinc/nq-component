...
class PointerHandler {
  ...
  handleEvent(e) {
    ...
    switch (e.type) {
      ...
      case 'pointerup':
        this.listener(e.type);
        this.cache.delete(e.pointerId); // remove event listener if all pointer is up
  ...
  getMouseCoordinates(x, y) {
    ...
    const offsetY = y - bound.top; // real size of the element to the screen stretch or compress

    const clientWidth = this.element.clientWidth;
    const clientHeight = this.element.clientHeight; // original size of the element in canvas this is the view port

    const width = this.element.width ? this.element.width : clientWidth;
    const height = this.element.height ? this.element.height : clientHeight; // normalize the mouse coordinates from 0 to 1 to get percentage

    const nX = offsetX / clientWidth;
    const nY = offsetY / clientHeight; // multiply to the view port size to convert percentage to real position
